...
def calcular_indicadores(df):
    if len(df) >= 26:
        df['RSI'] = talib.RSI(df['close'].astype(float), timeperiod=14) # Asegurar float
        bbands = talib.BBANDS(df['close'].astype(float), timeperiod=20, nbdevup=2, nbdevdn=2, matype=0) # Asegurar float
        df['BB_UPPER'] = bbands[0]
        df['BB_MIDDLE'] = bbands[1]
        df['BB_LOWER'] = bbands[2]
        macd = talib.MACD(df['close'].astype(float), fastperiod=12, slowperiod=26, signalperiod=9) # Asegurar float
        df['MACD'] = macd[0]
        df['MACD_signal'] = macd[1]
        df['MACD_hist'] = macd[2]
        periodo_base = 9
        if len(df) >= periodo_base * 2:
            sqrt_periodo = int(periodo_base**0.5)
            df['HULMA'] = talib.MA(df['close'].astype(float), timeperiod=periodo_base).rolling(window=sqrt_periodo).mean() # Asegurar float
            df['HULMA'] = talib.MA(df['HULMA'].astype(float), timeperiod=sqrt_periodo).rolling(window=sqrt_periodo).mean() # Asegurar float
        else:
